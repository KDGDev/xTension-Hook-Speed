...
public class MyActivity extends Activity {
	private static final String TAG = "xTensionTest";
		...
		Thread th = new Thread(new Runnable() {
			...
			public void run() {
				...
				for (int j = 0; j < TESTS; j++) {
					...
					t1time[j] = SystemClock.currentThreadTimeMillis() - t1;
				...
				final String result = String.format(
						"\n" +
						"Реальный вызов = %sus (%.2f per call)\n" +
						"Перехваченный = %sus (%.2f per call)\n" +
						"xTension = %sus (%.2f per call)\n" +
						"\nrelative results:\n" +
						"Перехваченный/Реальный вызов=%.2f\nxTension/Реальный вызов=%.2f\nПерехваченный/xTension=%.2f",
